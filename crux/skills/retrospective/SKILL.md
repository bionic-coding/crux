...
Root defense: the journal contract (§4 of `docs/CLAUDE.md`) reserves the `## [` line prefix for headings and `log-work` enforces it, so a compliant tree cannot contain a planted in-body anchor. These validity rules are defense-in-depth for non-compliant history or adversarially-authored journal entries.
...
- [ ] **Journal reflective sections** — scan every `docs/journal/*.md` entry in the window for sections that answer "what was hard", "what I'd do differently", "what surprised me", or equivalent. Copy verbatim quotes with path + anchor.
