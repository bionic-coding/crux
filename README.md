<!-- generated-from: README.md@sha256:bbda15b132667ad2a4163a3b70c2833f6a1542e514d9a482f06533c9d1f3e3ac; model: claude-fable-5.1; date: 2026-09-15 -->
# Crux
...
Restart the session, then say **init docs**. Full details, plus Codex and OpenCode setup, are in [Install](#install) below.

## About this repository

This repository is **generated** from a private development repository on every release (see `.generated`). Each release lands here as a single squash commit. Please **file issues** — pull requests cannot be merged, because the next release would overwrite them.
...
crux bundles a **role-based agent layer**: **ten** Claude Code subagents that *operate* the skills. Each carries a `tools` allowlist that **structurally enforces** what it may and may not do — separation-of-duties by tooling, not by instruction.
...
- **[CODEX.md](./CODEX.md)** — Codex installation for humans and agents.
- **[OPENCODE.md](./OPENCODE.md)** — OpenCode machine-wide and per-project setup for humans and agents.
- **[CHANGELOG.md](./CHANGELOG.md)** — what each release added.

## License & status

MIT licensed (see [LICENSE](./LICENSE)). v3.16.1 — see [CHANGELOG.md](./CHANGELOG.md) for the per-release breakdown. Issues welcome; the public repo is regenerated on every release, so pull requests there cannot be merged.
