<!-- generated-from: README.md@sha256:de2b2f7f54dd463b30725ccfb96cc17406b91752b6c43ac10e9091d06ab71d65; model: claude-fable-5.1; date: 2026-09-14 -->
# Crux
...
Codex and OpenCode installation are covered under [Other hosts](#other-hosts) below.

## About this repository

This repository is **generated** from a private development repo on every release (see `.generated`). Each release lands here as a single squash commit. Please file **issues** here — they are welcome and read — but do not open pull requests: the next release would overwrite them.

---

## What crux manages

**60 skills** across seven concerns — plus a default-on, derived **arch** surface (the current-state architecture map, built on demand by `derive-arch`) and the default-on **observations** concern (what the code already does, evidenced and ratified rather than decided) — plus a **10-agent operator layer** (see **Agents** below), under a single naming convention — no `crux-` prefix. Skills are organized into four bundles: docs suite, core agent-team, verification, infrastructure.
...
- **[OPENCODE.md](./OPENCODE.md)** — OpenCode manual setup for humans and agents.
- **`bionic/CLAUDE.md`** (after `init docs`) — the operational schema Claude reads. Authoritative.

## License & status

MIT licensed (see [LICENSE](./LICENSE)). v3.15.0 — see [CHANGELOG.md](./CHANGELOG.md) for the per-release breakdown. Issues welcome; the public repo is regenerated on every release, so pull requests there cannot be merged.
