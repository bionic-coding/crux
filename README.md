<!-- generated-from: README.md@sha256:9e402730b0a99964db18613558a4ddc0d486eea4bbb7ddb08ee1135423c14979; model: claude-fable-5.1; date: 2026-09-10 -->
# Crux
...
This repository is **generated** from a private development repo on every release (see `.generated`). Each release lands here as a single squash commit — there is no development history to browse and nothing to branch from. Please **file issues** for bugs and requests; pull requests opened here would be overwritten by the next release and cannot be merged.
...
That's the whole flow — the second command installs the plugin named `crux` from the marketplace you just added (also named `crux`) — install and upgrade alike (re-run `/plugin install crux@crux` after `/plugin marketplace update crux` to pick up a new version). Installation does **not** touch your project's docs folder; you bootstrap that yourself. Restart the session so the plugin's skills and agents register, then say:
...
| **Summary (fixed)** | 1 | Completion report (stats, key decisions, links) **and archival** — every cycle ends with `archive-promptbook` so finished books don't straggle in `active/` |
...
| "audit docs" | Run the integrity checks across every enabled concern and auto-fix safe drift; `audit docs --migrate` upgrades an older docs tree |
...
MIT licensed (see [LICENSE](./LICENSE)). v3.11.1 — see [CHANGELOG.md](./CHANGELOG.md) for the per-release breakdown. Issues welcome; the public repo is regenerated on every release, so pull requests there cannot be merged.
