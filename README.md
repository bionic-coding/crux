...
Codex and OpenCode setup are covered under [Other hosts](#other-hosts) below.

## About this repository

This public repository is **generated** from a private development repo on every release (see `.generated`). Each release lands here as **one squash commit** plus a tag. Please **file issues** for bugs, questions, and requests — pull requests against this repo would be overwritten by the next release, so they can't be merged here.
...
The full mental model and conventions live in [USER_GUIDE.md](./USER_GUIDE.md). The operational schema Claude follows lives in `bionic/CLAUDE.md` (created on first run). The tree's location is recorded in `.bionic.yml`; an existing `docs/` tree keeps working unchanged.
...
|---|---|---|
| `init-docs` | "init docs" — first run in a repo | Bootstraps the seven-concern `bionic/` tree plus the default-on arch spine + `bionic/CLAUDE.md` + `manifest.yml` + the meta-ADR. |
...
| `install-codex-agents` | "install the Crux agents in Codex" / "refresh Codex agents" — Codex users only | Installs or refreshes the ten generated Crux role agents into a target repo's `.codex/agents/`; no-clobber (`--force` required to overwrite a modified role). |
...
| Skill | Say this / when | What it does |
|---|---|---|
| `audit-docs` | "audit docs" / before a release | ~45 integrity checks across all concerns; auto-fixes safe drift. |
...
crux is also a supported Codex distribution target: the same ten role agents are generated straight from `crux/agents/` into Codex-native `.codex/agents/*.toml`, and skills are consumed as-is (no conversion needed). Install through Codex's own plugin marketplace flow:
...
Then say **"install the Crux agents in Codex"** to write the ten namespaced `crux_*` role agents into your project's `.codex/agents/` (the `install-codex-agents` skill — it never overwrites a conflicting agent file without `--force`). Everything else — `init docs`, the skill catalog, the cycle workflow — works identically to Claude Code.

### OpenCode (manual setup)

There is **no native Crux OpenCode marketplace package yet** — OpenCode support is a **manual, preview-grade setup** for now (a public OpenCode distribution channel is a deliberately deferred future decision). The steps below wire the full 60-skill + 10-agent set into OpenCode V2 (`opencode2`) from a public clone. They work from the published repo — no dev checkout required.
...
5. **Symlink every generated agent** into `~/.config/opencode/agents/` (OpenCode has no `paths` escape hatch for agents — the files must physically exist there). The glob catches all ten roles — `wayfinder` and any future role — automatically. Symlinks, not copies, so a later regenerate needs no re-symlink:
   ...
   > **These symlinks expose a V2-only projection — run the roles with `opencode2`.** The generated files carry V2's `permissions` array. A V1 `opencode` runner does not read that key, so it loads each role with every `deny` dropped. The role then grants what the file meant to refuse. Nothing warns you: the file parses, the role appears, and the restriction is gone.
...
7. **Verify:**
   ...
   ```bash
   opencode2 debug agents   # should list the 10 Crux roles alongside the built-ins
   ```
   ...
   V2 has no skill-listing command; `opencode2 debug` offers exactly `agents`, `config` and `paths`. Verify skills by asking for one instead — say *"what's next"* and expect `cleanup-campsite` to run.
...
- **Your own `bionic/adrs/`** — start at `ADR-0000`, walk forward in number order. The "why" of every architectural commitment in your project.
- **[CHANGELOG.md](./CHANGELOG.md)** — what each release added.

## License & status

MIT licensed (see [LICENSE](./LICENSE)). v3.9.0 — see [CHANGELOG.md](./CHANGELOG.md) for the per-release breakdown. Issues welcome (this repo is generated on release, so please file issues rather than PRs — see [About this repository](#about-this-repository)).
