<!-- generated-from: USER_GUIDE.md@sha256:aeea54b20814a90c9cdd1b467e98fabb4755685e842f8c8f2211b22f4ef90b4f; model: claude-fable-5.1; date: 2026-09-09 -->
# Working with `bionic/` — the crux user guide

crux keeps your project's documentation in `bionic/` — seven concerns, plus two default-on surfaces (`arch` and `observations`) — maintained by the `crux` Claude Code plugin. **You do not write these docs by hand.** You curate, decide, and discuss. Claude does the bookkeeping.
...
crux turns a `./bionic/` folder in your repository into a maintained knowledge base that you and Claude share. There are four moving parts:
...
2. **Skills — natural language, no slash commands.** "propose an ADR", "process inbox", "audit docs", "start a cycle", "forge a skill". Each is triggered by a phrase routed through the skill's description. → [What to say to Claude](#what-to-say-to-claude) (the repository `README.md` has the full catalog)
...
The split is enforced by tooling, not etiquette — a librarian *cannot* write, a reviewer *cannot* edit, a developer *cannot* re-delegate. The agents embed the craft disciplines they need (TDD, verification, debugging, two-stage review) so they're self-contained. They become dispatchable once the plugin is installed or upgraded and the session is restarted.
...
- **"Audit docs"** → runs the integrity check suite (~54 checks) across every enabled concern. Auto-fixes safe drift (counts, dates, missing index rows); surfaces broken cases for your decision.
...
`${CLAUDE_PLUGIN_ROOT}` is the plugin's installed root — Claude Code sets it inside sessions, so the path above resolves wherever the marketplace installed the plugin. The examples below abbreviate that invocation to `crux-env` — define your own shell alias if you use it often. The commands below are the whole interface.
...
You've just opened a crux-managed repo. Read in this order:

1. **`bionic/CLAUDE.md`** (long — skim the first sections first) — the operational schema. The single source of truth for what lives where and who edits what.
...
You're in a fresh repo with `bionic/` just initialized (say *"init docs"* if you haven't). To start using it:
