<!-- generated-from: USER_GUIDE.md@sha256:386e41786c8c0e8e637393724a446a70b68c08cfcfb9516cc55caaeb1213ab46; model: claude-fable-5.1; date: 2026-09-21 -->
# crux User Guide
...
This guide is for humans. If you are an LLM agent picking up a crux-managed repo, read `bionic/AGENTS.md` in that repo — that's the operational schema `init-docs` writes.
...
Then, in your project, say **"init docs"** to bootstrap the `bionic/` tree. See [Plugin and schema](#plugin-and-schema) for upgrade notes.
...
**Codex users:** say **"install the Crux agents in Codex"** after installing the plugin. The installer writes the ten namespaced `crux_*` roles to `~/.codex/agents/` by default; an explicit `--repo-root` selects one project's `.codex/agents/` directory instead. An unchanged refresh is a no-op; changed or stale managed files require `--force` after review. Relocating the plugin appears as drift because skill bindings use absolute paths. `--check --project-context <repo>` reports managed drift and project agents that shadow personal roles.
...
Configure which extractors run by editing `code.extractors:` in `bionic/manifest.yml`. Day-one extractors: Elixir, fallback (header-comment scrape). More languages ship as extractor plugins in later releases.
...
Everything Claude does is backed by Python scripts under `${CLAUDE_PLUGIN_ROOT}/scripts/` — stdlib-only for the docs tooling; the multi-model substrate (below) adds PEP 723-declared dependencies. **You almost never run these directly** — the skills invoke them for you — but knowing they exist helps when something looks off.

`${CLAUDE_PLUGIN_ROOT}` is the plugin's installed root. Claude Code sets it inside sessions; if you want to run a script from your own shell, point it at the marketplace-installed copy of the plugin.
...
The full byte-level spec for the secrets store and CLI contract is in `bionic/AGENTS.md` §13 of your project's tree.
...
- **`artifact_prefix`** — brand artifact ids so they're distinguishable across repos: with `artifact_prefix: "CRX"`, new books and ADRs get ids prefixed `CRX-` (e.g. `CRX-ADR-0012`). Existing artifacts are never renamed.
...
Full contract: `bionic/AGENTS.md` §14 in your project's tree.

---

## Where to look first

Coming into a crux-managed repo for the first time, read in this order:

1. **`bionic/AGENTS.md`** (skim §1–§7 first) — the operational schema. The single source of truth for what lives where and who edits what.
...
| A script exits `2` | That's an environment problem (missing `uv`, PyYAML, or a tree-sitter grammar), never a docs problem — read the remediation message. |
| Whole `bionic/` tree feels broken | Run *"audit docs"* — integrity checks across all concerns. |

---

## Plugin and schema

crux maintains its documentation tree at `schema_version 5` (the `bionic/` layout). The plugin itself lives at the marketplace-installed root, `${CLAUDE_PLUGIN_ROOT}`.

In Claude Code, install or upgrade with `/plugin marketplace add bionic-coding/crux` and `/plugin install crux@crux`. In Codex, use `codex plugin marketplace add bionic-coding/crux` and `codex plugin add crux@crux`.

After upgrading from a release before 3.19.0, say *"audit docs --migrate"* even when `schema_version` is already `"5"`. The migration converts tracked `CLAUDE.md` files to `AGENTS.md`. When both files exist at one scope, it preserves unique blocks and removes only exact duplicates under the same heading path. If a heading conflicts or a block cannot retain its heading ancestry, the migration keeps both source files and writes `.instruction-migration-preview.md`. Reconcile each named block in a JSON resolution file that copies `source_hashes` from `.instruction-migration-receipt.json`, then say *"audit docs --migrate using the resolution file at `<path>`"*. It reports untracked and private suppressors without editing them.

Codex users can install the ten Crux role agents personally with the `install-codex-agents` skill.

`init-docs` also writes a project-local copy of this guide into your repo; re-running `init-docs --force` overwrites that copy.
